...
import com.rizesky.nps.analyzer.PacketAnalyzerAbstract;
import com.rizesky.nps.analyzer.PacketAnalyzerLoader;
import jpcap.packet.*;
...
class TablePane extends JPanel implements ActionListener,ListSelectionListener
{
	...
	void loadProperty(){
		if(NetworkPacketSniffer.JDProperty.getProperty("TableView")!=null){
			//get all menus
			...
			//load ptoperty
			StringTokenizer status=new StringTokenizer(NetworkPacketSniffer.JDProperty.getProperty("TableView"),",");
	...
	void saveProperty(){
		...
		if(viewStatus.length>0){
			...
			for(int i=1;i<viewStatus.length;i++)
				...
				NetworkPacketSniffer.JDProperty.put("TableView",buf.toString());
		}
	}
}
